...
const CardCarrito = (props) => {
  ...
  const navigate = useNavigate()
    return (
      <Col md={4}>
        <div onClick={() => navigate(`/productos/${props.product.id}`)}>
          <Card className='card borde' style={{ width: '18rem' }}>
            ...
            <Card.Body>
              ...
              <Button variant="danger" onClick={() => setContextState({type: ActionTypes.SetEliminar, value: props.product.id})}>Eliminar del carrito</Button>
            </Card.Body>
          </Card>
          </div>
      </Col>
